...
$(function() { // DOM이 준비되면
	...
	$(document).on("click", ".deleteReply", function(e) {
		console.log("deleteReply Clicked");

		let no = $(this).attr("data-no");
		let fno = $("#replyForm input[name=fno]").val();
		
		let params = "replyNo=" + no + "&fNo="+ fno;
		console.log(params);

		let writer = $(this).parent().prev().prev().find("span").text();

		let $replyForm = $("#replyForm").slideUp(300);

		let result = confirm(writer + "님이 작성한 " + no + "번글을 삭제할까여?");
			...
			$.ajax({
				...
				"success": function(resData) {
					...
					$.each(resData, function(i, v) {
						...
						+           `<span><strong>${v.mid}</strong></span>`
						+           '<small>'
						+             `${strDate}`
						+           '</small>'
						+         '</div>'
						+         `<pre class="mt-2 mb-2">${v.ccon}</pre>`
						+         '<div class="text-end">'
						+           `<button class="btn btn-outline-success btn-sm me-1 modifyReply" data-no="${v.cno}">수정</button>`
						+           `<button class="btn btn-outline-warning btn-sm me-1 deleteReply" data-no="${v.cno}">삭제</button>`
						+           `<button class="btn btn-outline-danger btn-sm" onclick="reportReply('${v.cno}')">신고</button>`
						+         '</div>'
						+       '</div>'
	...
	$("#replyList").on("click", ".modifyReply",function(e) {
		...
		$("#replyForm").is(":visible"); // 보이는 상태
		$("#replyForm").css("display", "none").removeClass("d-none").slideDown(300);
